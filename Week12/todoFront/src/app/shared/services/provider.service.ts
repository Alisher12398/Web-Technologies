...
import {ITaskList, ITaskShort, ITaskLong, ITask} from '../models';
...
export class ProviderService extends MainService {
  ...
  getTaskList(task_list: ITaskList): Promise<ITaskList>{
    return this.get(`http://localhost:8000/api/task_lists/${task_list.id}/`, {})
  }
  ...
  createTask(task_list:ITaskList, name: any, created_at: any, due_on: any, status: any): Promise<ITask>{
    return this.post(`http://localhost:8000/api/task_lists/${task_list.id}/tasks/`, {
      name: name,
